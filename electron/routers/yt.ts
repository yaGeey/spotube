...
export const ytRouter = router({
   getPlaylist: publicProcedure
      .input(z.string())
      .query(async ({ input: playlistId }): Promise<YtPayloadWithPlaylist | undefined> => {
         // check cache
   ...
   batchFromSpotifyTracks: publicProcedure
      ...
      .query(async ({ input }): Promise<YtPayload[][]> => {
         // console.log(chalk.blue(`Searching for ${input.length} Spotify tracks on YouTube`))
               ...
               limit(async () =>
                  pRetry(
                     async () => {
                        ...
                        const lastFMData = await getFullLastFMInfo(item.query.artist, item.query.title)
                        let lastFm: null | LastFM = null
                        if (lastFMData.artist || lastFMData.track || lastFMData.album)
                           ...
                           .map((v) => {
                              return prisma.youtubeVideo.upsert({
                                 ...
                                 update: {
                                    ...
                                    ...(lastFm && { lastFm: { connect: { id: lastFm.id } } }),
                                 },
                                 create: {
                                    id: v.id,
                                    title: v.title,
   ...
   getPlaylists: publicProcedure.query(() => prisma.youtubePlaylist.findMany()),

   createPlaylist: publicProcedure.input(z.string()).mutation(async ({ input: playlistId }) => {
      // fetch playlist metadata
      const playlistRes = await googleYoutube.playlists.list({
         part: ['snippet'],
         id: [playlistId],
      ...
      // create playlist entry
      return await prisma.youtubePlaylist.create({
         data: {
            id: playlistId,
            title: playlist.snippet?.title || 'Unknown Title',
            owner: playlist.snippet?.channelTitle || 'Unknown Owner',
